...
class PluginsManagerFileSystemTests: PluginsDataControllerEventTestCase {
    ...
    func testAddAndDeletePlugin() {
        ...
        copyWithConfirmation(of: plugin, destinationPluginPath: destinationPluginPath, handler: { (plugin) -> Void in
            newPlugin = plugin
        })
        ...
        XCTAssertEqual(PluginsManager.sharedInstance.plugin(for: testPluginName)!, newPlugin, "The plugins should be equal")
        ...
        XCTAssertEqual(PluginsManager.sharedInstance.plugin(for: testPluginName)!, originalPlugin, "The plugins should be equal")
        XCTAssertEqual(PluginsManager.sharedInstance.plugins().count, 1, "The plugins count should be one")
    }
