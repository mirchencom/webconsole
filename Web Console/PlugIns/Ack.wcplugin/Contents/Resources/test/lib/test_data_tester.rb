    ...
    def self.test_file_hashes(files_hash, test_files_hash)
      ...
      test_files_hash.keys.each do |file_path|
        ...
        if test_file.file_path != file.file_path
          puts "The file paths should match. #{test_file.file_path} #{file.file_path}"
          return false
        end
          
        if test_file.display_file_path != file.display_file_path
          puts "The display file paths should match. #{test_file.display_file_path} #{file.display_file_path}"
          return false
        end
        ...
        test_file.lines.zip(file.lines).each do |test_line, line|
          ...
          if test_line.number != line.number
            puts "The line numbers should match. #{test_line.number} #{line.number}"
            return false
          end

          if test_line.matches.count != line.matches.count
            puts "The match counts should match. #{test_line.matches.count} #{line.matches.count}"
            return false
          end

          test_line.matches.zip(line.matches).each do |test_match, match|
            if test_match.text != match.text
              puts "The match text should match. #{test_match.text} #{match.text}"
              return false
            end
          end
